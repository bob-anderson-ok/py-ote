...
def convertTimeStringToTime(timeStr):
    ...
    """
    try:
        if timeStr[ 0] != '[': return -1.0
        if timeStr[-1] != ']': return -1.0
...
def improveTimeStep(outliers, deltaTime):
    validIntervals = [deltaTime[i] for i in range(len(deltaTime)) if not i in outliers]
