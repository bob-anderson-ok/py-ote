# build-exe.ps1 — produce a single PyOTE.exe via PyApp.
#
# Prerequisites (one-time on the build machine):
#   1. uv              — already installed as part of normal PyOTE development.
...
#   3. GNU Windows target + rust-mingw:
#        rustup toolchain install stable-x86_64-pc-windows-gnu
#      (This brings the `rust-mingw` component which provides the matched
#      dlltool and libgcc_eh.a used below. Do NOT skimp with just
#      `rustup target add` — that only installs rust-std and linking fails.)
#
# Usage (from the project root):
#   powershell -ExecutionPolicy Bypass -File scripts\build-exe.ps1
#
# Output:  dist\PyOTE.exe

$ErrorActionPreference = "Stop"
Set-Location -Path (Join-Path $PSScriptRoot "..")

Write-Host "==> Building PyOTE wheel with uv..."
uv build
if ($LASTEXITCODE -ne 0) { throw "uv build failed" }

$wheel = Get-ChildItem -Path dist -Filter "pyote-*-py3-none-any.whl" |
         Sort-Object LastWriteTime -Descending |
         Select-Object -First 1
if (-not $wheel) { throw "no pyote wheel found in dist/" }
Write-Host "    wheel: $($wheel.FullName)"

Write-Host "==> Configuring PyApp..."
$env:PYAPP_PROJECT_PATH       = $wheel.FullName
$env:PYAPP_EXEC_SPEC          = "pyoteapp.pyote:main"
...
$dst = Join-Path $PWD "dist\PyOTE.exe"
