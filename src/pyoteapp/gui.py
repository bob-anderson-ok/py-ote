...
class Ui_MainWindow(object):
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.redoFindEvent.setText(_translate("MainWindow", "Re-do fit"))
        ...
        self.allowNewVersionPopupCheckbox.setToolTip(_translate("MainWindow", "<html><head/><body><p><span style=\" font-weight:600; color:#aa0000;\">What\'s new:</span></p><p><span style=\" color:#000000;\">Timestamp error lines now distinguish between \'cadence jitter\' and \'dropped frames\' by using an additional color.</span></p><p><span style=\" color:#000000;\">Cadence jitter happens when high frame rates create a situation where frame recording cannot keep up and the traffic control implemented by the computer changes the arrival/timestamp time of frames so that some appear a little earlier and others a little late.</span></p><p><span style=\" color:#000000;\">Cadence jitter is defined as a timing discrepancy of between 20% and 80% of a frame time and is represented by a yellow line.</span></p><p><span style=\" color:#000000;\">Timing discrepancies greater than 80% of a frame time continue to be shown as a red line and are assumed to come from dropped frames. In any case, they deserve attention.</span></p><p><span style=\" font-weight:700; color:#0000ff;\">Older:</span></p><p><span style=\" color:#000000;\">An option has been added to the Analysis tab that allows a possible single point drop to be evaluated as a potential event.</span></p><p><span style=\" color:#000000;\">A single point event (defined as only 1 or possibly 2 readings in the lightcurve that are statistically below baseline) cannot be evaluated by the usual methods because the duration of the event is ambiguous (it only has an upper limit of a reading time, but insufficient information to compute a lower limit) and the noise level of a single point is zero or meaningless (your choice) so error bar calculations are disrupted as well. But it is still possible to ask the question \'is that drop just due to noise or is it more likely caused by a short duration occultation?\'. The tool to answer that question is the False-Positive test which addresses that exact question. The new option makes it possible to subject a single point to a False-Positive examination.</span></p></body></html>"))
