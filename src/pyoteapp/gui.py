...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        self.showTimingCorrectionCurvesButton = QtWidgets.QPushButton(self.widget)
        self.showTimingCorrectionCurvesButton.setObjectName("showTimingCorrectionCurvesButton")
        self.horizontalLayout_13.addWidget(self.showTimingCorrectionCurvesButton)
        ...
        spacerItem3 = QtWidgets.QSpacerItem(500, 13, QtWidgets.QSizePolicy.Minimum, QtWidgets.QSizePolicy.Expanding)
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.showTimingCorrectionCurvesButton.setToolTip(_translate("MainWindow", "<html><head/><body><p>If values have been entered in the two boxes to the right (Ast dist, and speed), it is possible to compute the diffraction lightcurve (based on a straight-edge model) that will \'seen\' by the camera.</p><p>We refer to this as the \'underlying lightcurve\': it is the theoretical intensity of the light diffracted at the occulting edge (assumed straight) as a function of time, i.e. as the shadow of the occulting body passes the observation position.</p><p>Diffraction intensity has a wavelength dependence. In PyOTE, we compute a generic diffraction curve composed of light in the 400nm to 600nm range (using 5 nm steps) with equal intensities at each included wavelength. The region of the diffraction curve that is of most interest to us is +/- 1 Fresnel unit on either side of the geometrical shadow point. Fortunately, in that region, there is very little difference between a narrow bandpass and a wide bandpass of light wavelengths, so there is no practical need to specify a more realistic bandpass in terms of either shape or extent. The primary effect of a wide bandpass is to greatly reduce the oscillations seen in a single wavelength diffraction curve (i.e., the one that is most frequently used to show diffraction effects).</p><p>If the star has a finite disk, the effect on the underlying lightcurve is usually very significant and apparent, \'integrating\' away most of the characteristic diffraction curve shape and moving toward what is often referred to as a penumbral shape. It also moves the value of the intensity at the geometrical shadow point from 25% of full scale to something close to 50% of full scale.</p><p>Clicking this button brings up a display of the underlying light curve shape. Note that the effect of the camera exposure time, which integrates the underlying light curve to produce what we refer to as the camera response, is not shown in the plots if a camera exposure time has not yet been established, otherwise, the plots will show the effect of the camera exposure time on the underlying lightcurve.</p></body></html>"))
        self.showTimingCorrectionCurvesButton.setText(_translate("MainWindow", "Underlying lightcurve"))
